...
My coworker should include the outcomes of the F-statistic and Shapiro Wilk-normality test. A one-way ANOVA test assumes that there is a normally distributed response and that there are equal variances between groups. Therefore, in order to determine that a one-way ANOVA was the correct choice and that our data fit the assumptions needed. An F-statistic is the ratio between group variance to within group variance, and tests for the equal variance assumption. The Shapiro Wilk-normlaity test determines how well the standarized sample quantiles fit the standard normal quantiles, and if they behave normally or not. These two tests are important 
